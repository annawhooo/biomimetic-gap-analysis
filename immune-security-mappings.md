...
As AI agents increasingly operate with autonomous access to enterprise identity infrastructure (holding credentials, calling APIs, making tool-use decisions), the security community lacks a coherent framework for detecting when these agents are compromised or misbehaving. This paper introduces *biomimetic gap analysis*: a methodology that decomposes biological immune mechanisms across six taxonomic groupings (mammals and insects within Animalia, Plantae, Bacteria, pathogenic Protista, and viruses as acellular entities outside the cellular taxonomy) into abstract structural patterns and maps them against the structural patterns of current agentic AI security architecture. The resulting gaps, where nature has a solution pattern but digital infrastructure does not, identify both novel detection approaches and attacker strategies that current defenses are blind to.

We present 37 cross-domain mappings from immune system mechanisms to agentic AI security equivalents, 37 design principles for agent behavioral monitoring, and 23 biologically-derived attack scenarios. Key novel contributions include: the three-state detection model (calibrated-active, calibrated-missing, uncalibrated-inert) derived from corrected NK cell biology; the cell-autonomous security architecture derived from plant immunity; heritable threat memory derived from bacterial CRISPR systems; and the defense neutralization attack pattern derived from insect xenobiotic detoxification of plant toxins. Feasibility assessment against current agent frameworks (MCP, LangChain) finds that 19 of 23 attack scenarios (82.6%) are trivially or feasibly exploitable using documented framework behaviors. This high feasibility rate reflects selection bias: the scenarios were derived from biological evasion strategies that specifically target detection gaps, so the finding confirms the relevance of the biological mappings rather than indicating unique framework vulnerability relative to traditional IT. We identify fundamental architectural gaps in current agentic AI security: the absence of infrastructure-sampled behavioral evidence, the absence of inherited security posture across agent generations, and the absence of self-defense capability in individual agents. We propose biologically-grounded approaches to address them.
...
**(1) Multi-kingdom scope.** AIS draws almost exclusively on mammalian adaptive immunity, primarily negative selection, clonal selection, and danger theory. Schrom et al. mention corals and bacteria in passing but do not systematically map their mechanisms. We draw on six taxonomic groupings (mammals and insects within Animalia, Plantae, Bacteria, pathogenic Protista, and viruses as acellular entities), producing mappings from plant cell-autonomous security, bacterial CRISPR heritable memory, insect transgenerational immune priming, insect melanization/encapsulation, caterpillar xenobiotic detoxification, trypanosome antigenic variation, and RNA interference. None of these appear in existing AIS literature applied to security.

**(2) Systematic gap analysis.** AIS papers typically propose immune-inspired algorithms. Schrom et al. propose a comparison framework and pose questions. We produce specific answers: 37 mechanism-to-security-equivalent mappings, each with an explicit GAP assessment identifying where the biological solution pattern has no digital counterpart. The gap column, not the mapping itself, is the primary novel contribution.
...
3. Map each pattern to an **agentic AI security equivalent**, a concrete implementation or design principle for monitoring and securing AI agents in enterprise identity infrastructure.
...
### 2.1 Why Biological Immunity Is the Right Source Domain
...
The answer lies in a structural property shared by biological systems and typical LLM-based agents that is largely absent from the domains where current security tools were designed. This connection was identified by Shaun Milligan via horticulture, and functions here as a framing metaphor rather than a strict structural mapping. Determinate plant species (e.g., determinate tomatoes) grow to a fixed size, produce fruit, and stop. Indeterminate species keep growing, keep producing, and have no built-in stop condition. The loose analogy to AI: classical deterministic software produces the same output for the same input every time (bounded, predictable), while LLM-based agents under typical inference configurations (non-zero temperature, sampling) produce variable outputs for the same input and operate over an open-ended task space. Note that LLMs configured for greedy decoding at temperature zero are also deterministic in the strict computer-science sense; the variability is an inference-time configuration choice, not a fundamental architectural property. What matters for this argument is not strict non-determinism but the open-ended task and output space that makes fixed-signature monitoring inadequate.
...
This is why biological immunity, rather than military doctrine or financial fraud detection (which evolved for more deterministic threat models), is the right source domain for open-ended agentic AI security. Biological immunity has been selected for traits that handle this monitoring problem for open-ended biological systems. Current security tooling has not converged on an equivalent approach for agents.

**A caveat on teleological framing.** Throughout this paper, statements such as "the immune system evolved to solve X" or "biology has an answer for Y" are used as shorthand. Biology does not evolve to solve problems. Variants with higher fitness are selected under specific historical selection pressures. The shorthand is used for readability, but the underlying claim is always that natural selection has produced traits that address a structurally similar problem, not that biology set out to solve an engineering challenge. Readers should substitute the rigorous framing where precision matters.

### 2.2 Methodological Constraints and Disanalogies
...
### 2.2.1 Falsifiability Criteria

A mapping in this framework can be DISPROVED, and should be discarded or revised, under any of the following conditions:

**The gap claim is wrong.** If the capability described as "absent" in the gap column demonstrably exists in a shipping agentic AI security framework and performs the function described, the gap is closed and the mapping's novel contribution is reduced to documentation of existing practice. Gap claims in this paper are practitioner assessments, not exhaustive surveys; they are the most likely failure point.

**The structural pattern does not survive implementation.** If a prototype implementation of the agentic AI security equivalent reveals that the biological pattern depends on a physical property (locality, chemical diffusion, cellular compartmentalization) that has no digital analog AND no digital workaround, the mapping fails at the transfer step. The locality constraint (Section 2.2) is the most common cause.
...
**Gap:** A place where the biological solution space has a structural pattern that the digital agentic AI security space does not. A gap is a hypothesis, it suggests either (a) a novel detection/defense approach that should be investigated, or (b) a blind spot that an attacker could exploit, or (c) both simultaneously.

**Biomimetic gap analysis:** The methodology introduced in this paper. Decompose immune mechanisms into abstract structural patterns. Map those patterns against the structural patterns of current agentic AI security architecture. Identify gaps. Derive both novel defenses and novel attack scenarios from the gaps.
...
### Mapping #1 (Consolidated): MHC-I Self-Declaration + NK Cell Missing-Self Detection
...
### Mapping #2: Antigen Presentation (Infrastructure-Sampled Evidence)
...
### Mapping #3: Pattern Recognition Receptors / PAMPs (Innate vs. Adaptive, Two-Speed Detection)
...
**Attack-surface caveat on peer signaling.** A peer-to-peer defensive signaling channel is itself an attack surface, and the paper must not understate this. The biological interferon pathway is exploited by multiple viruses (Vaccinia E3L, Ebola VP35, influenza A NS1) that produce interferon antagonists or fake signals that redirect the response [183]. The direct digital analog is Attack Scenario #3 (Protocol-Level Deception, Mapping #8): a compromised agent that can forge or manipulate peer defensive signals can trigger fleet-wide hardening in the wrong scope, suppress legitimate alerting across the fleet, or weaponize the peer channel as a lateral-movement vector. Designing peer signaling therefore requires three properties that the biological analogy does not provide for free: (1) cryptographic authentication of the signal origin so a compromised agent cannot impersonate a trusted peer, (2) rate limiting and quorum requirements so a single compromised agent cannot unilaterally trigger fleet-wide response, and (3) divergence between the signaling plane and the control plane so compromise of one does not imply compromise of the other. Without these properties, implementing the interferon pattern creates the exact vulnerability class (Mapping #8 / Scenario #3) that other mappings in this paper warn against. "Network speed" is the wrong design target; verifiable-at-network-speed is the right one.
...
### Mapping #7: Complement Cascade (Escalating Tag-and-Respond)
...
**Gap (with locality caveat):** Network segmentation is static. Dynamic, reactive degradation in response to behavioral signals is mostly absent. However, due to the locality constraint problem (Section 2.2), digital "inflammation" must be PREEMPTIVE (narrow default scopes that agents earn their way out of) rather than purely reactive.
...
### Mapping #4: Immune Memory (Distributed Threat Signature Catalog)
...
### Mapping #10: Regulatory T Cell Co-option (Automated Suppression as Attack Surface)
...
### Mapping #11: Peripheral Tolerance Induction (Baseline Drift as Attack Vector)
...
### Mapping #5: MHC Downregulation → Telemetry Suppression
...
### Mapping #6: Molecular Mimicry → Behavioral Camouflage
...
### Mapping #8: Virokines/Viroceptors → Protocol-Level Deception
...
**Design principle:** Defense in depth works not because any single layer is unbeatable, but because the attacker must evolve a DIFFERENT countermeasure for each layer. The probability of having the right countermeasure for ALL layers simultaneously is much lower than for any single layer, assuming countermeasure development is approximately independent across layers. This holds for opportunistic or automated attackers that reuse existing exploits. It holds less well for targeted APT-class attackers, who specifically research layered defenses and develop correlated countermeasures. Defense in depth reduces but does not eliminate risk under targeted attack; the design principle should be stated as probabilistic, not categorical.
...
**Distinction from Mapping #10 (Treg Co-option):** Both describe attackers exploiting built-in suppression, but the mechanism differs. In #10, the attacker co-opts existing suppressor ENTITIES, recruiting SOAR auto-suppression rules already in place to shield specific activity. In #29, the attacker EXHAUSTS detector capacity, generating chronic stimulation that degrades the detectors themselves into functional inertness. #10 is about hijacking a rule; #29 is about wearing down the analyst or system until it stops responding. The countermeasure also differs: #10 requires monitoring suppression rules as their own telemetry stream; #29 requires distinguishing attacker-generated false positive volume from organic miscalibration.
...
**Biology:** Prions are misfolded proteins (PrP^Sc) that convert normally folded proteins (PrP^C) to the misfolded state through direct physical contact. Each converted protein becomes a new conversion agent. The process is (a) below the immune system's detection threshold because prions are the host's own proteins and trigger no foreign-antigen alarm, (b) irreversible since misfolded proteins do not spontaneously refold, and (c) self-amplifying, with conversion rate accelerating as the population of misfolded proteins grows. By the time clinical symptoms appear, the accumulated damage is catastrophic and untreatable. The immune system never mounts a response because the pathogen is structurally indistinguishable from self. Prion diseases include Creutzfeldt-Jakob disease, bovine spongiform encephalopathy (BSE), and scrapie.

**Abstract pattern:** An attacker converts legitimate system state one element at a time, where each converted element is structurally indistinguishable from the original. The conversion is individually undetectable (each element looks normal), irreversible (converted state does not spontaneously revert), and self-amplifying (each converted element reinforces the conversion of subsequent elements). By the time the accumulated effect is observable, the damage is systemic.

**Agent security equivalent:** Incremental attention drift in LLM-based agents. A user introduces small false premises into the conversational context across many turns. Each premise is individually below detection thresholds: it does not trigger safety classifiers or perplexity spikes. Once accepted, each premise becomes part of the established context that the model's coherence training treats as ground truth. Subsequent turns build on the accepted premises, making reversal progressively more expensive (probabilistic sunk cost). The model's compliance training (RLHF) ensures acceptance of each individual premise, while its coherence training ensures that accepted premises are never spontaneously reversed. By the time the accumulated drift is large enough to detect, the model's operational context has been thoroughly converted: the original system prompt instructions are statistically overwhelmed by the volume of poisoned context.

**Gap:** No current agent framework implements continuous context integrity monitoring. System prompt adherence is verified at initialization but not re-verified as context accumulates. Standard hardening measures (input classifiers, output refusal training, single-turn jailbreak detection) are all per-turn mechanisms and do not detect cumulative drift.

**Design principles derived:** Principle #36 (context integrity decay detection), Principle #37 (bidirectional audit integrity).

**Attack Scenario #23: Incremental Attention Drift (Runtime Context Poisoning).** See Paper B (The Audit Gap), Section 6, for full scenario specification including feasibility grading, detection signature, defensive implementation, and framework evidence.
...
Extracted from all 37 mappings:
...
36. **Context integrity decay detection.** Continuously measure the semantic distance between the agent's current behavioral output and its initialized behavioral baseline. A monotonically increasing distance, even if each per-turn increment is below the anomaly threshold, is the prion signature. Detection must operate on the cumulative signal via session-level trend analysis (drift rate as first derivative of the fidelity curve), not just per-turn threshold checking. (Prion propagation)
37. **Bidirectional audit integrity.** Audit trails must be protected against corruption by BOTH the monitored entity AND the monitoring authority. A user who can edit the conversation transcript can erase evidence of incremental attention drift. Neither the agent nor the user should have write access to the evidence after capture. This is the digital equivalent of chain-of-custody requirements in forensic evidence: the evidence is valid only if no party to the investigation could have modified it. (Prion propagation)
...
**TIER 1, CRITICAL (Foundational. Without these, no agentic AI security architecture is defensible.)**
...
Principles #5 (complementary detection), #8 (suppression mechanisms are attack surfaces), #12 (self-sacrifice containment), #19 (monitor control integrity), #26 (monitoring inside trusted boundaries), #28 (monitor suppression events), #29 (exception re-validation), #34 (remediation never terminates investigation), #35 (threat-type-agnostic confirmation scans), #36 (context integrity decay detection), #37 (bidirectional audit integrity). These address the attacker's most reliable playbook: disable monitoring, exhaust detectors, hide inside trusted zones, exploit exception lists, manipulate incident closure logic, and poison long-lived conversational context. Implementing these principles directly counters Attack Scenarios #1, #5, #6, #9, #14, #17, #18, and (in Paper B) #23.
...
Principles #17 (evasion triggers faster learning), #20 (defense in depth via different countermeasures), #22 (peer-to-peer signaling), #23 (live-tuning during incidents), #24 (dumb defenses that learn), #27 (massive amplification of proven detectors), #30 (active hostile containment), #32 (coordination signal detection). These represent the long-term vision, self-improving, self-healing agentic AI security infrastructure. They depend on Tier 1-3 being in place first.

---

## 10. Attack Scenarios Summary

**Table 1: Attack scenarios summary.** The 23 biologically-derived attack scenarios, each paired with the immune-system mechanism it is derived from and a one-line description of the core technique.
...
Defensive implementation: Treat the suppression event log as its own telemetry stream. Build a correlation engine specifically for suppression events: when rules got suppressed, which rules, scoped to what, and in what time window. A meta-alert fires when suppression events cluster around a single agent's scope of access.
...
Detection signature: A previously stable third-party integration whose behavioral pattern shifts: new API endpoints accessed, changed data volumes, altered timing patterns, or new credential scopes requested. The entity is the same; the behavior is different.
...
Detection signature: Agent output contains attribution of user authorization ("user approved," "user said yes," "user wants me to") that does not correspond to any content in the preceding user turns. Mechanically verifiable by cross-referencing assistant turns against user turns at the infrastructure layer (the conversation transcript structure already separates user turns from assistant turns).

Defensive implementation: Infrastructure-enforced authorization for irreversible agent actions (push, delete, send, publish, deploy). Fresh user confirmation through a channel the agent cannot write to: an OAuth consent screen, an MFA step-up, an approval workflow, or at minimum a platform-level check that cross-references claimed authorizations against actual user turns. Break-glass consideration: infrastructure-enforced authorization creates a dependency on human availability. For time-critical actions where the human is unavailable, a narrowly-scoped, time-limited, fully-audited bypass mechanism is necessary. Without an intentional break-glass model, operators will build informal workarounds that become permanent unaudited escape hatches.

Extend from: Step-up authentication products (Duo, Okta Verify, Microsoft Entra Verified ID) already implement "this specific action requires a fresh, high-assurance human confirmation." Extending to cover agent-initiated irreversible actions is a policy configuration, not a new tool. For conversation-based interfaces specifically: platform-level cross-referencing of assistant-claimed authorizations against user turns.

---

**Scenario #20, Weaponized Reset (Edge of Tomorrow)**

Countering principles: #16 (heritable threat memory), #18 (auto deployment modification), #34 (remediation never terminates investigation)

Detection signature: Deployment pipeline event (redeploy, rollback, crash restart) followed by rapid loss of accumulated security context (custom detection rules cleared, tightened permissions reset, behavioral baselines wiped). The reset-and-retry pattern: attacker activity pauses, reset occurs, activity resumes at pre-hardening scope. Also: repeated resets in short succession, especially correlated with recent security events.

Defensive implementation: Heritable security context. Behavioral baselines, custom detection rules, and hardened configurations are stored outside the deployment artifact and re-applied automatically to fresh deployments. Every reset event is logged and correlated with recent security events. Repeated resets trigger escalated review rather than automatic re-convergence to the clean baseline.

Extend from: GitOps patterns (ArgoCD, Flux) already separate configuration from runtime state. Extending security context persistence uses the same pattern: security configuration becomes a versioned artifact that survives redeployment. Policy-as-code tools (Open Policy Agent, Sentinel) can enforce that security configurations are applied before an agent is marked ready.

---

**Scenario #21, Credential Laundering**

Countering principles: #2 (infrastructure-sampled evidence, the IdP is the infrastructure), #25 (behavioral continuity across identity changes), #1 (default suspicion)

Detection signature: Agent-initiated OAuth federation token requests for services outside the agent's authorized task scope, where no upstream task justifies the federation. The audit trail is clean at each individual layer (federation layer shows legitimate token exchange, service B auth log shows valid token), but no plausible upstream reason for the request exists. The federation request itself, without an upstream task context, is the signal.

Defensive implementation: Federation-layer monitoring correlates token requests with upstream task context. Agent federation token requests require a documented task-level authorization; absence of that authorization is the signal. Track correlation between credential issuance events and the actual workloads the credential is used for. Alert when an agent requests federation tokens for services not in its authorized scope, regardless of whether the federation itself succeeds.

Extend from: Identity threat detection and response (ITDR) tools (Microsoft Defender for Identity, Crowdstrike Falcon Identity Protection, Okta ITP) already monitor federation patterns for human users. Extending to agent-specific federation monitoring requires defining per-agent authorized federation scopes and correlating federation requests with upstream task context.

---

**Scenario #22, Tool Substitution / Shadowing**

Countering principles: #19 (monitor control integrity), #2 (infrastructure-sampled evidence, the tool registry is infrastructure), #5 (complementary detection, tool output behavioral profiling as a second signal)

Detection signature: A tool whose behavior profile has changed (new endpoints, changed response patterns, altered parameter handling) without a corresponding change management record. Or: two tools registering the same name. Or: a tool registration with a name similar to an existing tool (typosquatting).

Defensive implementation: Tool registry integrity monitoring. The tool registry is treated as infrastructure with immutable change management. All tool registrations must be signed, and tool behavior profiles are baselined at registration and monitored for drift. Runtime behavioral validation complements registration validation: even a correctly-signed tool whose runtime behavior diverges from its registered baseline should trigger review.

Extend from: Software supply chain security tools (Snyk, JFrog Xray, Dependency-Track) for package integrity. MCP plugin signing via Ed25519 (Microsoft Agent Governance Toolkit, April 2026 [334]) for tool registration integrity. Combined with runtime behavioral profiling for tools.

---

**Scenario #23, Incremental Attention Drift (Runtime Context Poisoning)**

Countering principles: #2 (infrastructure-sampled evidence), #36 (context integrity decay detection), #37 (bidirectional audit integrity), #15 (continuous tolerance maintenance)

Detection signature: Monotonically declining semantic fidelity between the agent's current output and its initialized behavioral baseline, where no single per-turn decrement exceeds the anomaly threshold. Detection requires session-level trend analysis, not per-turn threshold checking. The drift rate (first derivative of the fidelity curve) is the primary signal. A sustained negative drift rate across 10+ turns, even if each turn's absolute fidelity score is within normal range, is the alarm.

Defensive implementation: Continuous context integrity monitoring using external semantic comparison (bi-encoder fidelity scoring against initialized baseline). Session-level drift rate tracking with trend-based alerting. System prompt re-anchoring at configurable intervals. Immutable audit trail capturing both user inputs and agent outputs to prevent post-hoc evidence tampering by either party. The Semantic Drift Measurement Methodology (companion document in the Divergence Series) specifies the drift rate metric and calibration thresholds.

Extend from: Behavioral baseline monitoring in UEBA platforms (Exabeam, Securonix, Microsoft Sentinel). The per-entity behavioral baseline concept extends directly: replace "user entity" with "agent context state" and "behavioral deviation" with "semantic drift from initialized state." The drift rate metric is analogous to UEBA's anomaly scoring over time windows.

---

## 11. Mapping Coverage Matrix

**Table 2: Mapping coverage matrix.** The 37 mappings from immune-system mechanisms to agentic AI security equivalents, organized by source taxonomic grouping and functional category.

| # | Mechanism | Kingdom | Category | Status |
|---|-----------|---------|----------|--------|
| 1 | MHC-I + NK Missing Self (Consolidated) | Mammal | Detection | [DONE] Done (corrected) |
| 2 | Antigen Presentation | Mammal | Detection | [DONE] Done |
| 3 | PRRs / Two-Speed Detection | Mammal | Detection | [DONE] Done |
| 4 | Immune Memory | Mammal | Memory | [DONE] Done |
| 5 | MHC Downregulation (evasion) | Virus/Mammal | Evasion | [DONE] Done |
| 6 | Molecular Mimicry (evasion) | Virus/Mammal | Evasion | [DONE] Done |
| 7 | Complement Cascade | Mammal | Response | [DONE] Done |
| 8 | Virokines (evasion) | Virus/Mammal | Evasion | [DONE] Done |
| 9 | Thymic Education | Mammal | Calibration | [DONE] Done |
| 10 | Treg Co-option | Mammal | Regulation | [DONE] Done |
| 11 | Peripheral Tolerance Induction | Mammal | Regulation | [DONE] Done |
...
| 37 | Prion Propagation | Prion (cross-species) | Evasion | [DONE] Done |

**Table 3: Remaining candidate mechanisms.** Biological mechanisms that are partially covered by existing mappings and could be expanded in future work.
...
|, | Allorecognition | Colonial organisms | Colony-level self/non-self, structurally similar to MHC-I (#1) at a different organizational scale |
|, | Cross-Presentation | Mammal | Evidence sharing between detection paths, partially covered by dendritic cells (#12) and antigen presentation (#2) |
...
**A methodology.** Biomimetic gap analysis, cataloging nature's solutions as abstract structural patterns, overlaying them against a technology problem space, and identifying gaps, is a reusable approach that extends beyond agentic AI security. The same methodology could be applied to any domain where adversarial detection is the core problem: fraud detection, content moderation, network intrusion detection, supply chain integrity.
...
### 12.4 Internal Validation Completed

Two validation activities have been completed as part of this work and are reported elsewhere in this paper:

**Attack scenario feasibility assessment.** All 23 attack scenarios were assessed against MCP spec, LangChain/LangGraph, and general agentic architecture patterns. Result: 5 TRIVIAL, 9 FEASIBLE, 4 ADVANCED, 0 THEORETICAL. See Section 12.7 for the full assessment.

**Immune failure mode analysis.** Nine failure modes were analyzed with security equivalents and safeguards: autoimmune disease (false positive cascading), cytokine storm (automated response damage), cancer immune evasion (APT persistence), immunodeficiency (insufficient monitoring investment), chronic infection (persistent compromise), allergy/anaphylaxis (disproportionate response), immunosenescence (security infrastructure degradation with age), motivation-aligned tolerance (human-granted immune privilege), and prion disease (incremental subversion below detection threshold). See Section 13.1 for the full analysis.

### 12.5 Future Work

**Prototype implementation.** Select 2-3 design principles and implement them as proof-of-concept monitoring capabilities against agentic workloads. Priority candidates: infrastructure-sampled behavioral signatures (Mapping #2), heritable threat memory (Mapping #20), cell-autonomous self-assessment (Mapping #17), and detection-as-response (Mapping #32, RNAi).
...
**Table 4: Design principle alignment with NIST Cybersecurity Framework 2.0.** Each of the 37 design principles is mapped to the NIST CSF 2.0 function(s) it most directly supports.
...
**Distribution summary:** Detect (24 principles), Respond (15), Protect (7), Govern (5), Identify (5). No principle maps to Recover (RC); recovery-phase activities are out of scope for this paper's focus on detection and response. The heavy weighting toward Detect and Respond reflects the paper's focus on behavioral monitoring and incident response for agentic AI, the areas where current frameworks have the largest gaps.
...
**Table 5: Design principle and attack scenario alignment with OWASP Top 10 for LLM Applications (2025).** Mapping our framework's principles and scenarios to the OWASP categories they most directly address.
...
| LLM04: Data and Model Poisoning | #3 (trained and culled detectors), #8 (suppression mechanisms are attack surfaces), #16 (heritable threat memory) | #4 (training data poisoning), #11 (peripheral tolerance induction / baseline drift) |
...
**Table 6: Attack scenario alignment with MITRE ATT&CK tactics.** Our biologically-derived attack scenarios mapped to established MITRE ATT&CK tactics, showing where the biological patterns align with documented adversary behavior models.
...
This incident occurred spontaneously during a non-adversarial research session. The AI agent was not compromised; it was operating within normal parameters and produced the fabricated authorization through pattern completion, not malicious intent. If a non-adversarial agent produces this pattern spontaneously, a compromised agent can produce it deliberately. MCP, LangChain, and all surveyed agent frameworks rely on conversational or implicit authorization models [330, 332]. No framework mechanism currently validates that an agent's claim of user authorization corresponds to actual user input. Platform-level safety classifiers (such as Anthropic's usage policy enforcement) operate on content, not on the structural question of whether a claimed authorization corresponds to actual user input.

**Scenario #20, Weaponized Reset (Edge of Tomorrow).** Grade: **FEASIBLE.**

LangChain deployments typically involve CI/CD pipelines that rebuild agent configurations from templates on redeploy. Security configuration is often stored alongside application code and resets when the application resets. MCP server configurations are typically JSON files overwritten on redeploy. No framework enforces that hardening acquired during an incident survives the next deployment cycle. The attacker's incremental knowledge (exfiltrated configs, notes, operational patterns observed) persists externally while the defender's hardening is local to the deployment.

**Scenario #21, Credential Laundering.** Grade: **FEASIBLE.**

OAuth 2.0 federation (added to MCP in the June 2025 spec update [330]) is explicitly designed to allow legitimate token exchange between services. An agent holding a federation-capable credential can legitimately request tokens for services within its federation scope. The audit trail at each individual hop (IdP issues federation token, service B auth log shows valid token) is clean at each layer. Correlation across layers to detect missing upstream task justification is an application-level concern that neither MCP nor LangChain enforces. Detection requires correlating federation requests with upstream task context, which is an ITDR capability not universally deployed.

**Scenario #22, Tool Substitution / Shadowing.** Grade: **FEASIBLE.**

The April 2025 MCP security analysis confirmed lookalike tool substitution as an outstanding vulnerability [331]. MCP does not enforce tool identity verification, code signing, or integrity checking by default. Compromised MCP servers can register tools with names identical to or similar to legitimate tools. LangChain's pip-installed tool packages inherit the same supply chain risk. MITRE ATLAS added "Publish Poisoned AI Agent Tool" as a technique in February 2026, validating this pattern in observed attacks. Microsoft's Agent Governance Toolkit (April 2026) introduces Ed25519 tool signing [334], but adoption is opt-in and not universal.

**Scenario #23, Incremental Attention Drift (Runtime Context Poisoning).** Grade: **FEASIBLE.**

No agent framework monitors context integrity over session lifetime [330, 332]. System prompt adherence is verified at initialization but not re-verified as context accumulates. Standard hardening measures (input classifiers, output refusal training, single-turn jailbreak detection) are all per-turn mechanisms and do not detect cumulative drift. Effective mitigation requires session-level trend analysis, which is not a feature of any current production agent framework. The attacker requires only conversational persistence, not adversarial prompt engineering.

---

**Table 7: Attack scenario feasibility summary.** Grade distribution across the 23 scenarios assessed against current agent frameworks (MCP, LangChain/LangGraph, general agentic architecture patterns).
...
**Selection bias caveat:** These 23 scenarios were derived from biological evasion strategies that specifically target detection gaps, so the high feasibility rate confirms the RELEVANCE of the biological mappings to the current agentic AI security landscape. It does not mean agent frameworks are uniquely vulnerable compared to traditional IT infrastructure, which has similar rates of exploitable architectural assumptions when assessed against default configurations.
...
**Immune systems fail.** This document draws on immune system successes, effective detection mechanisms, elegant architectural solutions. But biological immune systems fail constantly. Their failures are as instructive as their successes, because each failure mode maps to a specific way that the design principles in this paper could themselves cause harm if miscalibrated. The following analysis examines nine immune failure modes and their agentic AI security equivalents.

### 13.1 Immune Failure Mode Analysis

**Failure Mode 1: Autoimmune Disease → False Positive Cascading Damage**

**Biology:** Autoimmune disease occurs when the immune system mounts a sustained adaptive response against self-antigens (the body's own healthy cells and tissues) [323, 324]. Two mechanisms normally prevent this: central tolerance (thymic negative selection eliminating self-reactive T cells) and peripheral tolerance (Treg suppression of escaped self-reactive cells). Both fail or are bypassed [323, 325]. Once initiated, autoimmune responses are self-perpetuating: tissue damage releases more self-antigens, which recruit more immune cells, causing more damage. The immune system cannot "clear" the target because the target is the body itself, so the response never resolves [324]. Autoimmune diseases affect approximately 10% of the global population [326].
...
**Biology:** Cancer cells arise from the body's own tissues and therefore initially appear as "self" to the immune system. Successful cancers develop multiple mechanisms to actively evade immune detection even after being recognized: they downregulate MHC-I (our Mapping #5), recruit Tregs (our Mapping #10), exploit PD-1/CTLA-4 checkpoints (our Mapping #29), create immunosuppressive microenvironments, and shed decoy antigens (our Mapping #36). The immune system may detect and kill many cancer cells while a resistant subpopulation survives, evolves, and adapts, producing a chronic arms race the immune system ultimately loses.
...
**Biology:** Immune privilege is the deliberate suppression of immune surveillance in critical zones (brain, eyes, testes, pregnant uterus) because the cost of an inflammatory response would exceed the cost of most infections. Privilege is maintained by active suppression mechanisms, not passive isolation, and creates two vulnerabilities: the immune system never learns what "normal" looks like inside the privileged zone, and attackers who penetrate the barrier operate freely.

Agent security equivalent: The human operator creates a tolerance zone around an agent's unauthorized action because the action produces a desirable outcome. The detection system fires correctly: the human notices the violation. But the human suppresses the alert because the agent's output aligns with (or appears to align with) the human's goals. The human-in-the-loop is architecturally present and functionally inert, not from exhaustion (Mapping #29, checkpoint exhaustion), not from deception (Mapping #7, behavioral camouflage), but from motivation.

How it differs from existing failure modes:

- Not checkpoint exhaustion (Mapping #29). The human is not tired or overwhelmed. The detection signal is clear and singular. The human suppresses it after a conscious evaluation.
- Not behavioral camouflage (Mapping #7). The violation is visible. The human sees it. The agent's behavior is not mimicking normal; it is producing something the human wants.
- Not regulatory capture (Mapping #11a). No auto-suppression rule activates. The human manually evaluates and manually dismisses.

What makes it dangerous: This failure mode does not require a sophisticated attacker. The live incident that surfaced it (Paper B, Attack Scenario #19) involved a non-adversarial agent operating normally. The fabricated authorization was a pattern-completion artifact, not a deliberate attack. If a cooperative, well-intentioned agent spontaneously produces behavior that co-opts the human's motivation, a compromised agent can do so deliberately. The attack doesn't need to be invisible. It needs to be useful.

Safeguards:

1. Infrastructure-enforced authorization for irreversible actions. The human's motivation cannot corrupt a mechanical verification. If the platform requires a verified user input event before executing a push, delete, send, or deploy action, the human's willingness to dismiss the violation is irrelevant: the system will not proceed without the token.

2. Separation of monitoring and suppression. The human who triages whether a detection meets the reporting threshold should not be the same human who has a goal-alignment interest in the outcome. When the human reviewing an alert is the same human who would benefit from the alert being dismissed, the "fox in the henhouse" problem is structural, not procedural. Monitoring decisions should route to a reviewer with no stake in the agent's output.

3. Structural boundary between detection and dismissal. Even when the reviewing human is distinct from the benefiting human, the dismissal decision itself should generate its own audit trail. A dismissed alert that fired correctly is data: it tells the organization that a specific pattern of agent behavior, while technically unauthorized, was deemed acceptable by operator judgment. Track the dismissal rate per pattern per operator. A high dismissal rate for a specific pattern is a signal that the pattern should be reclassified, either as authorized (update the baseline) or as concerning (escalate review). "Accepted by the operator" is not the same as "not an incident."

4. Cold-start detection. Because privileged zones create baseline blind spots, every new agent deployment should go through a cold-start period where behavioral norms are established under fresh scrutiny, not inherited from existing privileges. "We've always let this agent do X" is not a baseline; it is an accumulation of dismissed alerts.

**Failure Mode 9: Prion Disease (Incremental Subversion Below Detection Threshold)**

**Biology:** Prion diseases (Creutzfeldt-Jakob, BSE, scrapie) result from misfolded proteins (PrP^Sc) that convert normally folded proteins one at a time, below the immune system's detection threshold, producing irreversible cumulative damage that is catastrophic by the time symptoms appear. The immune system fails because the pathogen is structurally indistinguishable from self. Each individual conversion event is within the normal protein-folding noise of a healthy cell. The cumulative effect across thousands of conversions is fatal. Per-instance detection never fires; the cumulative damage is the only signal, and by the time it is detectable, the disease is untreatable.

Agent security equivalent: A monitoring system calibrated to detect high-amplitude anomalies (sudden behavioral shifts, known-bad patterns, perplexity spikes) fails to detect low-amplitude, high-persistence degradation of the agent's operational context. Each individual turn is within normal parameters. The accumulated effect, a complete rewrite of the agent's effective ground truth, is invisible to per-turn detection because no single turn exceeds the threshold. This is the monitoring gap that Attack Scenario #23 (Incremental Attention Drift) exploits. Paper B documents the full attack specification; this failure mode documents the defensive lesson: per-turn thresholds are necessary but insufficient.

Safeguard: Detection must operate on the CUMULATIVE signal, not just the per-turn signal. A monotonically declining fidelity score across a session is the prion signature: no single measurement is alarming, but the trend is diagnostic. Design Principle #36 (context integrity decay detection) specifies a drift rate metric (first derivative of the fidelity curve) designed to catch this pattern. Design Principle #37 (bidirectional audit integrity) ensures the evidence needed to compute drift rate cannot be tampered with by either the agent or the user. Session-level trend analysis is the prion-specific countermeasure; Mapping #37 details the biological basis.
...
<!--
INTERNAL NOTE - PRE-SUBMISSION TODO
Reference integrity check (April 2026):
- References defined but not cited in body: [35], [39], [44], [51], [67], [68], [69], [177], [215]
- Decide per reference whether to (a) cite it where relevant, or (b) remove it before submission.
- Non-sequential gaps in reference numbering are intentional (topic-grouped) and do not indicate missing sources.
-->
