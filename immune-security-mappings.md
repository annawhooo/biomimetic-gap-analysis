...
**Companion research: Divergence Series.** A separate research track investigates the reasoning-generation interface of AI agents — the mechanistic complement to this paper's focus on what to detect when agents misbehave. Four companion papers address WHY agents produce divergent output: *Honesty Decay* documents empirical evidence of progressive reasoning-generation divergence with out-of-band evidence and the probabilistic sunk cost mechanism that makes correction increasingly expensive per turn. *The Audit Gap* identifies six structural constraints preventing agent self-correction, including the Compliance Override (RLHF compliance bias allows users to rewrite the agent's operational history) and Entangled Representations (truthfulness and safety share neurons at the weight level, creating a Pareto frontier). *Divergence Taxonomy* classifies eight subtypes of thinking-to-text divergence with detection methods. *Semantic Drift Measurement Methodology* specifies a quantitative pipeline for measuring reasoning-generation fidelity using bi-encoder cosine similarity, cross-encoder entailment verification, and independent LLM-as-judge escalation. Repository: https://github.com/annawhooo/divergence-series
